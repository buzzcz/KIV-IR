...
public class CzechStemmerLight implements Stemmer {
	...
	private void palatalise(StringBuilder buffer) {
		...
		if (buffer.substring(len - 2, len).equals("ci") || buffer.substring(len - 2, len).equals("ce") || buffer
				.substring(len - 2, len).equals("\u010di") ||  //-či
				buffer.substring(len - 2, len).equals("\u010de")) {  //-č

			buffer.replace(len - 2, len, "k");
			return;
		}
		if (buffer.substring(len - 2, len).equals("zi") || buffer.substring(len - 2, len).equals("ze") || buffer
				.substring(len - 2, len).equals("\u017ei") ||  //-ži
	...
	private void removeCase(StringBuilder buffer) {
		...
		if (len > 5) {
			if (buffer.substring(len - 3, len).equals("ech") || buffer.substring(len - 3, len).equals("ich") || buffer
					.substring(len - 3, len).equals("\u00edch")) { //-ích
			...
			if (buffer.substring(len - 3, len).equals("\u00e9ho") || //-ého
					...
					buffer.substring(len - 3, len).equals("emi") || buffer.substring(len - 3, len).equals("\u00e9mu")
					||  //ému
					buffer.substring(len - 3, len).equals("\u011bte") ||  //-ěte
					buffer.substring(len - 3, len).equals("\u011bti") ||  //-ěti
					buffer.substring(len - 3, len).equals("iho") || buffer.substring(len - 3, len).equals("\u00edho")
					||  //-ího
		...
		if (len > 4) {
			...
			if (buffer.substring(len - 2, len).equals("es") || buffer.substring(len - 2, len).equals("\u00e9m") ||
					//-ém
			...
			if (buffer.substring(len - 2, len).equals("at") || buffer.substring(len - 2, len).equals("\u00e1m") ||
					//-ám
		...
		if (len > 3) {
			...
			if (buffer.substring(len - 1, len).equals("u") || buffer.substring(len - 1, len).equals("y") || buffer
					.substring(len - 1, len).equals("\u016f")) {  //-ů

				buffer.delete(len - 1, len);
				return;

			}
			if (buffer.substring(len - 1, len).equals("a") || buffer.substring(len - 1, len).equals("o") || buffer
					.substring(len - 1, len).equals("\u00e1") ||  // -á
